...
class JobRequest(BaseModel):
    ...
    @classmethod
    def platforms_must_be_valid(cls, v):
...
class QueueRequest(BaseModel):
    ...
    @classmethod
    def jobs_must_not_be_empty(cls, v):
        if not v:
...
def _run_single_job(job_id: str, niche: str, platforms: list[str], upload_config: dict):
    _jobs[job_id] = {"status": "running", "step": "starting", "detail": ""}
...
def _run_queue_jobs(jobs: list[JobRequest], ids: list[str]):
    def on_progress(job_id: str, step: str, detail: str = ""):
        _progress_callback(job_id, step, detail)

    def on_job_complete(index: int, total: int, result: dict):
    ...
    run_queue(job_dicts, on_progress=on_progress, on_job_complete=on_job_complete)
...
def generate_video(req: JobRequest, background_tasks: BackgroundTasks):
    ...
    _jobs[job_id] = {"status": "queued", "step": "", "detail": ""}
    ...
    logger.info(f"Job queued: {job_id} | niche={req.niche} | platforms={req.platforms}")
    return {"job_id": job_id, "status": "queued"}


@router.post("/queue")
def queue_videos(req: QueueRequest, background_tasks: BackgroundTasks):
    ids = [str(uuid.uuid4())[:8] for _ in req.jobs]

    for job_id in ids:
        _jobs[job_id] = {"status": "queued", "step": "", "detail": ""}

    background_tasks.add_task(_run_queue_jobs, req.jobs, ids)

    logger.info(f"Queue started: {len(ids)} jobs | IDs: {ids}")
